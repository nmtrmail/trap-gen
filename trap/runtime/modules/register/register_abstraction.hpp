...
class RegisterCA : public RegisterAbstraction<DATATYPE> {
  ...
  void clock_cycle() {
    ...
    // Update list of stages containing NOPs after a stall.
    // Pipeline is currently stalled. Bubble grows forward every cycle.
    unsigned cur_nop_stages = this->m_nop_stages;
    if (this->m_stall_stage > -1) {
      this->m_nop_stages = (this->m_nop_stages << 1) | (1 << (this->m_stall_stage + 1));
    // Pipeline is resuming after a stall. Bubble decreases from the rear every
    // cycle.
    } else if (this->m_nop_stages) {
      this->m_nop_stages = (this->m_nop_stages << 1);
    }
    this->m_nop_stages &= (1 << this->m_num_stages) - 1;
    if (cur_nop_stages != this->m_nop_stages)
      this->m_has_to_propagate = true;

    // Execute propagation logic generated by TRAP.
    if (this->m_has_to_propagate) {
      if (this->m_clock_cycle_func != NULL) {
        this->m_has_to_propagate = (*this->m_clock_cycle_func)(m_values, temp_values, m_time_stamps, temp_time_stamps, this->m_stall_stage, cur_nop_stages, this->m_nop_stages);

        // Adopt new values.
        // The propagation logic, including stall logic, is hard-coded in
        // m_clock_cycle_func(). m_temp_values contain the new values to be
        // adopted.
        for (unsigned i = 0; i < this->m_num_stages; ++i) {
      ...
      // Assign stable value.
      if (!(this->m_nop_stages & (1 << (this->m_num_stages-1))))
...
void clock_cycle() {
  ...
  bool lock(void* instr, unsigned stage, unsigned latency) {
    assert(stage < this->m_num_stages);
    // The stage the calling instruction is currently in. This could be negative
