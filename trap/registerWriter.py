...
def getPipeClockCycleFunction(self, registerMaxBitwidth):
    ...
    -> registers.cpp:           regs_clock_cycle()
       ...
       register_abstraction.hpp. This, in turn, calls a callback function
       generated by TRAP in the registers.cpp file. The default behavior of the
       callback is to propagate each register from regsStage to wbStage one
       stage forward every cycle. In addition, wbStage is considered the stable
       value and fed back to all stages from [0..regsStage] as well as all
       stages from [wbStage+1..lastStage]. This is only for consistency, since
       the value should obviously neither be read nor written before regsStage
       (first read) or after wbStage (last write).

    2. Common feedback loops via <processor>Arch.py:<pipe>.setWbStageOrder():
       Aside from the default behavior in (1), the clock_cycle callback contains
       code honoring special forwarding paths: TRAP interprets <pipe>.
       ...
       the pipe forwarding paths take precedence in case of contention.
    ...
    -> registers.cpp:           <reg>_clock_cycle()
       ...
       in (2), but, since we have only one callback hook, the common pipe
       ...
       taken to re-define all feedback loops as required.

    This function generates the callback function containing default propagation
    logic as well as any forwarding specified by <pipe>.setWbStageOrder(
    ...
    # 1. Default behavior: Propagation to stages after wb.
    if wbStage == len(self.pipes) - 2:
      Code += """
        // Feed stable value to all stages after write-back. This is only for consistency.
        if (time_stamps[""" + str(wbStage) + """] >= time_stamps[""" + str(wbStage+2) + """]) {
          temp_values[""" + str(wbStage+2) + """] = values[""" + str(wbStage) + """];
      ...
      Code += """
        // Feed stable value to all stages after write-back. This is only for consistency.
        for (unsigned i = """ + str(wbStage+2) + """; i < this->m_num_stages; ++i) {
    ...
    # 1. Default behavior: Propagation to stages before and up to regsStage.
    if regsStage == 0:
      Code += """
    ...
    # 1. Default Behavior: Regular step-wise forward propagation.
    Code += """
      // Regular step-wise forward propagation.
      for (int i = """ + str(wbStage-1) + """; i >= """ + str(regsStage) + """; --i) {
        if (time_stamps[i] >= time_stamps[i+1]) {
          temp_values[i+1] = values[i];
          temp_time_stamps[i+1] = time_stamps[i];
          has_changes = true;
        }
      }
      """
    ...
    #pipeClockCycleTimeStampParam = cxx_writer.Parameter('time_stamps', cxx_writer.Type('sc_core::sc_time').makePointer())
    #pipeClockCycleTempTimeStampParam = cxx_writer.Parameter('temp_time_stamps', cxx_writer.Type('sc_core::sc_time').makePointer())
    ...
    pipeClockCycleFunction = cxx_writer.Function('regs_clock_cycle', cxx_writer.Code(Code), cxx_writer.boolType, [pipeClockCycleValueParam, pipeClockCycleTempValueParam, pipeClockCycleTimeStampParam, pipeClockCycleTempTimeStampParam])
...
def getRegisterClockCycleFunction(self, regName, wbStageOrder, registerMaxBitwidth):
    ...
    Code += """
      // Regular step-wise forward propagation.
      for (int i = """ + str(len(self.pipes)-2) + """; i >= 0; --i) {
        if (time_stamps[i] >= time_stamps[i+1]) {
          temp_values[i+1] = values[i];
          temp_time_stamps[i+1] = time_stamps[i];
          has_changes = true;
    ...
    #registerClockCycleTimeStampParam = cxx_writer.Parameter('time_stamps', cxx_writer.Type('sc_core::sc_time').makePointer())
    #registerClockCycleTempTimeStampParam = cxx_writer.Parameter('temp_time_stamps', cxx_writer.Type('sc_core::sc_time').makePointer())
