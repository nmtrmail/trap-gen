...
# @details  Instruction helper methods for the ARM7TDMI.
# @author   Luca Fossati
...
toGlue = toRotate & (((unsigned)0xFFFFFFFF) >> (32 - rotate_amm));
rotated = ((toRotate >> rotate_amm) & (((unsigned)0xFFFFFFFF) >> rotate_amm));
...
CPSR[CPSR_N] = ((resultSign & 0x0000000080000000LL) != 0);

//Update flag Z if the result is 0
CPSR[CPSR_Z] = (resultSign == 0);

//Update the C resultUnSign if a carry occurred in the operation
CPSR[CPSR_C] = (((operand1 ^ operand2 ^ ((unsigned)(resultSign >> 1))) & 0x80000000) != 0);

//Update the V flag if an overflow occurred in the operation
CPSR[CPSR_V] = ((((unsigned)(resultSign >> 1)) ^ ((unsigned)resultSign)) & 0x80000000) != 0;
""")
...
switch(shift_type){
    ...
    case 1:{
        ...
        if(shift_amm == 0){
            ...
            return ((unsigned) toShift) >> shift_amm;
        }
        break;}
    case 2:{
    ...
    case 3:{
        if(shift_amm == 0){
            return (((unsigned)toShift) >> 1) | ((CPSR[CPSR_C]) << 31);
        }
        else{
            // rotate right
...
opCode = cxx_writer.Code("""
unsigned curMode = CPSR[CPSR_mode];
switch(curMode){
    case 0x1:{
        //I'm in FIQ mode
        CPSR = SPSR[0];
...
updateAliases(curMode, CPSR[CPSR_mode]);
""")
...
switch(toMode){
    ...
    case 0x3:{
        //SVC mode
        REGS[13].update_alias(RB[15]);
        REGS[14].update_alias(RB[16]);
    break;}
    case 0x7:{
        //ABT mode
        REGS[13].update_alias(RB[17]);
        REGS[14].update_alias(RB[18]);
    break;}
    case 0xB:{
        //UND mode
        REGS[13].update_alias(RB[19]);
        REGS[14].update_alias(RB[20]);
    break;}
    default:{
...
if(i == 1){
    ...
    //register offset
    unsigned regVal = REGS[addr_mode1];
