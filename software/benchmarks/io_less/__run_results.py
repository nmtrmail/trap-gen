...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 3:
        raise Exception('Usage: processor_executable benchmark1 benchmark2 ...')
    if not os.path.exists(sys.argv[1]):
        raise Exception('Processor executable ' + sys.argv[1] + ' does not exist.')
    ...
    for benchmark in orderedBenchmarks:
        ...
        if not 'Program exited with value 0\n' in result:
            fileHandle.write('Benchmark ' + benchmark + ' failed. Wrong exit value.\n')
            continue
        for res in result:
